    ...
    for (i=0 ; i<N_BUFFS ; i++){
        if ((g+gladid)->buffs[i].timeleft > 0){
            ...
            //se atacar perde o buff
                ...
                if ((g+gladid)->action != ACTION_WAITING && (g+gladid)->action != ACTION_MOVEMENT && (g+gladid)->action != ABILITY_AMBUSH && (g+gladid)->action != ACTION_NONE)
...
void recordSteps(){
	...
	if (simtime == timeInterval){
		...
		for (i=0 ; i<nglad ; i++){
			...
			sprintf(buffer, "{\"name\":\"%s\",\"user\":\"%s\",\"id\":%i,\"lvl\":%i,\"xp\":%i,\"STR\":%i,\"AGI\":%i,\"INT\":%i,\"spd\":%.2f,\"as\":%.2f,\"cs\":%.2f,\"x\":%.2f,\"y\":%.2f,\"head\":%.1f,\"lockedfor\":%.2f,\"hp\":%.2f,\"maxhp\":%.2f,\"ap\":%.2f,\"maxap\":%.2f,\"action\":%i,\"message\":\"%s\",%s}",
				(g+i)->name,
				(g+i)->user,
				i, //thread num
				(g+i)->lvl, //lvl
