<?php
    ...
    if (isset($_POST['action'])){
        if ($_POST['action'] == 'SAVE'){
            ...
            $sql = "SELECT avg(g.mmr) AS mmr, r.log FROM gladiators g INNER JOIN reports r ON g.cod = r.gladiator INNER JOIN logs l ON l.id = r.log WHERE l.hash = '$loghash'";
            $result = runQuery($sql);
            $row = $result->fetch_assoc();
            $mmr = $row['mmr'];
            $log = $row['log'];

            $sql = "INSERT INTO stats (log, time, fireball, teleport, charge, block, assassinate, ambush, melee, ranged, win, avglvl, winnerlvl, duration, highstr, highagi, highint, avgmmr) VALUES ($log, now(), '$fireball', '$teleport', '$charge', '$block', '$assassinate', '$ambush', '$melee', '$ranged', '$win', '$avglvl', '$winnerlvl', '$duration', '$highstr', '$highagi', '$highint', '$mmr')";
            $result = runQuery($sql);
        }
