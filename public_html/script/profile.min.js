...
k+" de mestre para desbloquear este gladiador");$("#glads-container .glad-add").last().addClass("inactive")}})});$("#battle-container .tab").click(function(){$("#battle-container .tab").removeClass("selected");$(this).addClass("selected");f()});var l={battles:1,duels:1,favorites:1};$("#menu #battle").click(function(){$("#battle-container #prev").click(function(){l.battles--;f()});$("#battle-container #next").click(function(){l.battles++;f()});$("#match-find").prop("disabled",!0);$(this).hasClass("disabled")||
...
vstr:d.data("vstr"),vagi:d.data("vagi"),vint:d.data("vint"),mmr:d.data("mmr")};$.post("back_match.php",{action:"MATCH",id:c.id}).done(function(d){d=JSON.parse(d);d.push(c);var b=preBattleShow(d);runSimulation({ranked:!0}).then(function(c){"ERROR"!=c&&($("#pre-battle-show #tips").html("Batalha conclu\u00edda. Clique para visualizar"),clearInterval(b),$("#pre-battle-show").addClass("complete"),$("#pre-battle-show").click(function(){window.open("playback.php?log="+c);$("#fog").remove();$("#menu #battle").click()}));
a.kill()})})});var n=1;$("#ranking-container #prev").click(function(){n--;g()});$("#ranking-container #next").click(function(){n++;g()});$("#menu #ranking").click(function(){g()});$("#menu #messages").click(function(){$("#message-panel #full-message").remove();$("#message-panel .table").show();$.post("back_message.php",{action:"GET"}).done(function(a){a=JSON.parse(a);$("#message-panel .table").html("");for(var d in a)$("#message-panel .table").append("<div class='row'><div class='cell user'>"+a[d].nick+
"</div><div class='cell message'>"+a[d].message+"</div><div class='cell time'>"+getMessageTime(a[d].time)+"</div></div>"),"0"==a[d].isread&&$("#message-panel .table .row").last().addClass("unread");$("#message-panel .table .row").click(function(){var c=$("#message-panel .table .row").index($(this)),d=a[c].id,b=a[c].message,m=getMessageTime(a[c].time,!0),h=a[c].nick;c=a[c].picture;$("#message-panel").append("<div id='full-message'><div class='row head'><div class='image'><img src='"+c+"'></div><div class='user'>"+
