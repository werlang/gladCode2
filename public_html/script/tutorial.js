...
function showTutorial(){
	if (tutoState == 0){
		...
		if (text.search(/[\s]upgrade(INT|STR|AGI)[\s]{0,1}\([\d]{1,3}\)[;\s]/g) == -1){
			showDialog("Você deve inserir <span class='highlight'>upgradeSTR</span>, <span class='highlight'>upgradeAGI</span> ou <span class='highlight'>upgradeINT</span> no comportamento do gladiador. Só assim ele poderá melhorar um de seus atributos quando subir de nível",["upgrade?","Não sei como","OK"]).then( function(data){
				if (data == "upgrade?")
					window.open("https://gladcode.tk/docs.php#nav-up");
				if (data == "Não sei como"){
