...
import { blocks } from "./editor-blocks.js"

let user = false
login.wait().then(data => user = data)

let editor = false
let Blockly = false
waitFor(() => {
    return blocks.ready ? {
        editor: blocks.code.editor,
...
tutorial.lesson.start = async function(){
    ...
    if (data == "Nunca"){
        ...
        if (data == "Não"){
            await new Message({message: `Então vamos começar usando a programação em blocos.`}).show().click()

            setLang('blocks')
            blocks.toggle({ask: false, active: true})
            ...
            if (data == "SIM"){
                ...
                await new Message({message: `Agora que você sabe um pouco de lógica de programação, vamos prosseguir com o tutorial!`}).show().click()

                tutorial.next(true)
            }
            else{
                await new Message({message: `OK. Se mudar de ideia pode procurar nosso canal do Youtube. Vamos prosseguir com o tutorial!`}).show().click()
...
tutorial.lesson.skin = async function(){
    await new Message({message: `Configure a aparência do seu gladiador`}).show().click()
    editor.setValue("")
    $('#skin').click()
}
...
tutorial.lesson.checkStep = async function(){
    ...
    if (user.language == 'blocks'){
        ...
        if (search == -1){
            let data = await showDialog('Você ainda não inseriu <b>Passo para [frente]</b> dentro do bloco loop. Ele fica no menu de blocos gladCode > Movimentação. Experimente arrastá-lo para o seu código',["Como?","OK"])
            if (data == "Como?"){
                let info = {
                    message: "Vou lhe mostrar. Observe que coloquei o bloco <b>Passo para [frente]</b> dentro do loop. Agora clique no botão para testar o gladiador.",
                    code: '<xml xmlns="https://developers.google.com/blockly/xml"><block type="loop" id="_z|%q(O}n/R-cyoV1c-b" deletable="false" x="60" y="50"><statement name="CONTENT"><block type="step" id="$l8q8^?RbG#CFO+~!_S~"><mutation xmlns="http://www.w3.org/1999/xhtml" use-return="false"></mutation><field name="COMPLEMENT">FORWARD</field></block></statement></block></xml>'
                }
    
                const xmlDom = Blockly.Xml.textToDom(info.code)
                Blockly.mainWorkspace.clear()
                Blockly.Xml.domToWorkspace(xmlDom, Blockly.mainWorkspace)
                showDialog(info.message, ["Ok. Entendi"])
                tutorial.next()
            }
        }
        else {
        ...
        if (search == -1){
            ...
            await new Message({message: `Agora escolha um oponente para seu gladiador, e clique no botão para começar a batalha`}).show().click()
            tutorial.next()
            $('#test').click();
        }
    }
...
tutorial.lesson.oponent = async function(){
    if (user.language == 'blocks')
        await new Message({message: `Perfeito. Escolha um oponente para seu gladiador, e clique no botão para começar a batalha`}).show().click()
...
tutorial.lesson.watchCodeMove = async function(){
    if (user.language == 'blocks'){
        ...
        const xmlDom = Blockly.Xml.textToDom(code);
        Blockly.mainWorkspace.clear()
        Blockly.Xml.domToWorkspace(xmlDom, Blockly.mainWorkspace);
    }
    else{
    ...
    await new Message({message: `Agora analise esse código, tente entendê-lo e quando quiser teste o gladiador`}).show().click()
...
tutorial.lesson.showBackForth = async function(){
    ...
    await new Message({message: `Ok. Modifiquei seu código. Pode conferir se sua solução era mais ou menos assim. Teste o gladiador para prosseguir`}).show().click()
        ...
        $('.ace_gutter-cell').click( function(){
            ...
            setTimeout( function() {
                if ($('.ace_gutter-cell.ace_breakpoint').length){
                    new Message({message: `Muito bem. Agora teste seu gladiador.`}).show().click();
                    $('.ace_gutter-cell').off();
                }
            }, 500);
